<x-app-layout>
    <x-slot name="header">
        <div style="display:flex;align-items:center;gap:12px;">
            ...
            <div>
                <p style="font-size:12px;color:#94a3b8;font-weight:500;text-transform:uppercase;letter-spacing:1px;">
                    {{ auth()->user()->hasRole('admin') ? 'Administration' : 'Resident Services' }}
                </p>
        ...
        <div class="card fade-in fade-in-1" style="overflow:hidden;">
            <div style="background:linear-gradient(135deg,#7c3aed,#5b21b6);padding:24px 28px;">
                ...
                <p style="color:rgba(255,255,255,0.6);font-size:13px;margin-top:4px;">
                    ...
                    @else
                        Send a message directly to barangay admin
                    @endif
                </p>
            </div>
                        ...
                        <select name="receiver_id" class="form-input" required>
                            ...
                            @foreach($users as $user)
                                <option value="{{ $user->id }}" {{ old('receiver_id') == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }} {{ $user->hasRole('admin') ? '👤 Admin' : '🏠 Resident' }}
                                </option>
                            @endforeach
                        </select>
                        @error('receiver_id')
